<#
    ...
    .EXAMPLE
    .\Get-Authentication.ps1 -Computers Web01;Web02;Web03
    This will gather the WMI data from Web01, Web02, and Web03 using the credentials you are current logged in with.
    .EXAMPLE
    .\Get-Authentication.ps1 -Computers Web01;Web02;Web03 -User 'contoso\administrator'
...
ForEach ($sComputer in $global:aComputers)
{
	...
	#// Anonymous
	$oCollection = Get-WmiQuery -Namespace 'root\WebAdministration' -Query 'SELECT * FROM AnonymousAuthenticationSection' -Computer $sComputer
	If ($oCollection -ne $null)
	{
	    ForEach ($oInstance in $oCollection)
		{
			...
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Authentication' -Value 'Anonymous'
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Enabled' -Value $($oInstance.Enabled)
			$aObjects += @($oObject)
		}
	}

	#// Basic
	$oCollection = Get-WmiQuery -Namespace 'root\WebAdministration' -Query 'SELECT * FROM BasicAuthenticationSection' -Computer $sComputer
	If ($oCollection -ne $null)
	{
	    ForEach ($oInstance in $oCollection)
		{
			...
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Location' -Value $($oInstance.Location)
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Path' -Value $sPath
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Authentication' -Value 'Basic'
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Enabled' -Value $($oInstance.Enabled)
			$aObjects += @($oObject)
		}
	}
	...
	#// Digest
	$oCollection = Get-WmiQuery -Namespace 'root\WebAdministration' -Query 'SELECT * FROM DigestAuthenticationSection' -Computer $sComputer
	If ($oCollection -ne $null)
	{
	    ForEach ($oInstance in $oCollection)
		{
			...
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Location' -Value $($oInstance.Location)
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Path' -Value $sPath
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Authentication' -Value 'Digest'
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Enabled' -Value $($oInstance.Enabled)
			$aObjects += @($oObject)
		}
	}
	...
	#// Forms
	$oCollection = Get-WmiQuery -Namespace 'root\WebAdministration' -Query 'SELECT * FROM AuthenticationSection' -Computer $sComputer
	If ($oCollection -ne $null)
	{
	    ForEach ($oInstance in $oCollection)
		{
			$sPath = [string] $($oInstance.Path).Replace('MACHINE/WEBROOT/APPHOST/','')
			$sPath = [string] $sPath.Replace('MACHINE/WEBROOT/APPHOST','/')
			$sLocation = $($oInstance.Location)
	#		If ($sLocation -eq '')
	#		{
	#			$aString = $sPath.Split('/')
	#			$sLocation = $aString[0]
	#			$sPath = [string] $sPath.Replace($sLocation,'/')
	#		}
			
			$oObject = New-Object pscustomobject
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Computer' -Value $sComputer
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Location' -Value $sLocation
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Path' -Value $sPath
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Authentication' -Value 'Forms'
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Enabled' -Value $true
			$aObjects += @($oObject)
		}	
	}
	
	#// Windows
	$oCollection = Get-WmiQuery -Namespace 'root\WebAdministration' -Query 'SELECT * FROM WindowsAuthenticationSection' -Computer $sComputer
	If ($oCollection -ne $null)
	{
	    ForEach ($oInstance in $oCollection)
		{
			...
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Location' -Value $($oInstance.Location)
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Path' -Value $sPath
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Authentication' -Value 'Windows'
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Enabled' -Value $($oInstance.Enabled)
			$aObjects += @($oObject)
		}
	}
...
$aObjects | Export-Csv -Path '.\Get-Authentication.csv' -NoTypeInformation
Write-Host 'Done!'