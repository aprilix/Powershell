<#
    .SYNOPSIS
    Gets the request filtering settings for all web sites and virtual directories for all IIS7 servers.
    .DESCRIPTION
    Gets the request filtering settings for all web sites and virtual directories for all IIS7 servers. This scripts uses the root\WebAdministration WMI namespace. Requires administrator rights on the target server(s). The WMI connections are encrypted when possible.
    .EXAMPLE
    .\Get-RequestFiltering.ps1 -Computers Web01;Web02;Web03
    This will gather the WMI data from Web01, Web02, and Web03 using the credentials you are current logged in with.
    .EXAMPLE
    .\Get-RequestFiltering.ps1 -Computers Web01;Web02;Web03 -User 'contoso\administrator'
...
{
	Write-Host "Getting RequestFilteringSection data from $sComputer..." -NoNewline; Start-Timer
	$oCollection = Get-WmiQuery -Namespace 'root\WebAdministration' -Query 'SELECT * FROM RequestFilteringSection' -Computer $sComputer
	...
	ForEach ($oInstance in $oCollection)
	{
		...
		ForEach ($oAllowedQueryString in $oInstance.AlwaysAllowedQueryStrings.AlwaysAllowedQueryStrings)
		{
			$oObject = New-Object pscustomobject
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Computer' -Value $sComputer
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Path' -Value $sPath
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Type' -Value 'QueryString'
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Allowed' -Value $true
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Value' -Value $($oAllowedQueryString.QueryString)
			$aObjects += @($oObject)
		}
		ForEach ($oDeniedQueryString in $oInstance.DenyQueryStringSequences.DenyQueryStringSequences)
		{
			$oObject = New-Object pscustomobject
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Computer' -Value $sComputer
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Path' -Value $sPath
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Type' -Value 'QueryString'
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Allowed' -Value $false
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Value' -Value $($oDeniedQueryString.Sequence)
			$aObjects += @($oObject)
		}		
		ForEach ($oAllowedUrl in $oInstance.AlwaysAllowedUrls.AlwaysAllowedUrls)
		{
			$oObject = New-Object pscustomobject
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Computer' -Value $sComputer
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Path' -Value $sPath
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Type' -Value 'URL'
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Allowed' -Value $true
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Value' -Value $($oAllowedQueryString.Url)
			$aObjects += @($oObject)
		}
		ForEach ($oDeniedUrl in $oInstance.DenyUrlSequences.DenyUrlSequences)
		{
			$oObject = New-Object pscustomobject
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Computer' -Value $sComputer
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Path' -Value $sPath
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Type' -Value 'URL'
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Allowed' -Value $false
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Value' -Value $($oDeniedUrl.Sequence)
			$aObjects += @($oObject)
		}
		ForEach ($oFileExt in $oInstance.FileExtensions.FileExtensions)
		{
			$oObject = New-Object pscustomobject
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Computer' -Value $sComputer
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Path' -Value $sPath
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Type' -Value 'FileExtension'
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Allowed' -Value $($oFileExt.Allowed)
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Value' -Value $($oFileExt.FileExtension)
			#Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Value' -Value $('AllowUnlisted: ' + "$($oInstance.FileExtensions.AllowUnlisted)" + ', ApplyToWebDAV: ' + "$($oInstance.FileExtensions.ApplyToWebDAV)")
			$aObjects += @($oObject)
		}
		ForEach ($oRule in $oInstance.FilteringRules.FilteringRules)
		{
			$oObject = New-Object pscustomobject
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Computer' -Value $sComputer
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Path' -Value $sPath
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Type' -Value 'Rule'
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Allowed' -Value $false
			
			$aAppliesTo = @()
			ForEach ($oAppliesTo in $oRule.AppliesTo.AppliesTo)
			{
				$aAppliesTo += @($oAppliesTo.FileExtension)
			}
			$sAppliesTo = [string]::Join(',',$aAppliesTo)
			
			$aDenyStrings = @()
			ForEach ($oDenyString in $oRule.DenyStrings.DenyStrings)
			{
				$aDenyStrings += @($oDenyString.String)
			}
			$sDenyStrings = [string]::Join(',',$aDenyStrings)
			
			$aScanHeaders = @()
			ForEach ($oScanHeader in $oRule.ScanHeaders.ScanHeaders)
			{
				$aScanHeaders += @($oScanHeader.RequestHeader)
			}
			$sScanHeaders = [string]::Join(',',$aScanHeaders)
						
			$sValue = $('Name: ' + "$($oRule.Name)" + '; AppliesTo: ' + "$sAppliesTo" + '; DenyStrings: ' + "$sDenyStrings" + '; DenyUnescapedPercent: ' + "$($oRule.DenyUnescapedPercent)" + '; ScanAllRaw: ' + "$($oRule.ScanAllRaw)" + '; ScanHeaders: ' + "$sScanHeaders")
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Value' -Value $sValue
			$aObjects += @($oObject)
		}
...
$aObjects | Export-Csv -Path '.\Get-RequestFiltering.csv' -NoTypeInformation
Write-Host 'Done!'