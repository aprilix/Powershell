<#
    .SYNOPSIS
    Gets the mime type settings for all web sites and virtual directories for all IIS7 servers and counts the number of files that have that extension.
    .DESCRIPTION
    Gets the mime type settings for all web sites and virtual directories for all IIS7 servers and counts the number of files that have that extension. This scripts uses the root\WebAdministration WMI namespace. Requires administrator rights on the target server(s). The WMI connections are encrypted when possible.
    .EXAMPLE
    .\Get-MimeMapSettings.ps1 -Computers Web01;Web02;Web03
    This will gather the WMI data from Web01, Web02, and Web03 using the credentials you are current logged in with.
    .EXAMPLE
    .\Get-MimeMapSettings.ps1 -Computers Web01;Web02;Web03 -User 'contoso\administrator'
...
{
	Write-Host "Getting StaticContentSection data from $sComputer..." -NoNewline; Start-Timer
	$oCollection = Get-WmiQuery -Namespace 'root\WebAdministration' -Query 'SELECT * FROM StaticContentSection' -Computer $sComputer
	...
	If ($oCollection -ne $null)
	{
		ForEach ($oInstance in $oCollection)
		{
			...
			ForEach ($oMimeMap in $oInstance.StaticContent)
			{
				$oObject = New-Object pscustomobject
				$iCount = 0
				If ($($oMimeMap.FileExtension) -ne $null)
				{
					$sExt = $($oMimeMap.FileExtension).Replace('.','')
					$aFilesWithExtension = @($aFiles | Where-Object {($_.Extension -eq $sExt)} | Select Name, Extension)
					If ($aFilesWithExtension.Count -eq 0)
					{
						$iCount = 0
				...
				Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'MimeType' -Value $($oMimeMap.MimeType)
				Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Ext' -Value $($oMimeMap.FileExtension)
...
$aObjects | Export-Csv -Path '.\Get-MimeMapSettings.csv' -NoTypeInformation
Write-Host 'Done!'