<#
    ...
    .EXAMPLE
    .\Get-Impersonation.ps1 -Computers Web01;Web02;Web03
    ...
    .EXAMPLE
    .\Get-Impersonation.ps1 -Computers Web01;Web02;Web03 -User 'contoso\administrator'
    ...
    .EXAMPLE
    .\Get-Impersonation.ps1 -Computers Web01;Web02;Web03 -User 'contoso\administrator' -Password 'LetMeIn123' -OutputCsvFilePath 'C:\Iis7NtfsPermissions.csv'
...
ForEach ($sComputer in $global:aComputers)
{
	...
	$oCollection = Get-WmiQuery -Namespace 'root\WebAdministration' -Query 'SELECT * FROM IdentiitySection' -Computer $sComputer
	...
	If ($oCollection -ne $null)
	{
		ForEach ($oInstance in $oCollection)
		{
			...
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'UserName' -Value $([string] $oInstance.UserName)
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Impersonate' -Value $([bool] $oInstance.Impersonate)
			$aObjects += @($oObject)
		}
	}
	Else
...
$aObjects | Export-Csv -Path '.\Get-Impersonation.csv' -NoTypeInformation
Write-Host 'Done!'