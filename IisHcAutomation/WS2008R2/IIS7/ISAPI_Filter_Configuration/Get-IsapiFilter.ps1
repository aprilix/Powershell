<#
    ...
    .EXAMPLE
    .\Get-IsapiFilter.ps1 -Computers Web01;Web02;Web03
    This will gather the WMI data from Web01, Web02, and Web03 using the credentials you are current logged in with.
    .EXAMPLE
    .\Get-IsapiFilter.ps1 -Computers Web01;Web02;Web03 -User 'contoso\administrator'
...
ForEach ($sComputer in $global:aComputers)
{
	...
	$oCollection = Get-WmiQuery -Namespace 'root\WebAdministration' -Query 'SELECT * FROM IsapiFiltersSection' -Computer $sComputer
	...
	ForEach ($oInstance in $oCollection)
	{
		...
		ForEach ($oIsapiFilter in $oInstance.IsapiFilters)
		{
			...
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Name' -Value $([string] $oIsapiFilter.Name)
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'IsapiPath' -Value $([string] $oIsapiFilter.Path)
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'PreCondition' -Value $([string] $oIsapiFilter.PreCondition)
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Enabled' -Value $([string] $oIsapiFilter.Enabled)
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'EnableCache' -Value $([string] $oIsapiFilter.EnableCache)
			$aObjects += @($oObject)
		}		
	}
}
$aObjects | Format-Table -AutoSize
$aObjects | Export-Csv -Path '.\Get-IsapiFilter.csv' -NoTypeInformation
Write-Host 'Done!'