<#
    ...
    .EXAMPLE
    .\Get-UrlAuthorizationRules.ps1 -Computers Web01;Web02;Web03
    ...
    .EXAMPLE
    .\Get-UrlAuthorizationRules.ps1 -Computers Web01;Web02;Web03 -User 'contoso\administrator'
    ...
    .EXAMPLE
    .\Get-UrlAuthorizationRules.ps1 -Computers Web01;Web02;Web03 -User 'contoso\administrator' -Password 'LetMeIn123' -OutputCsvFilePath 'C:\Iis7NtfsPermissions.csv'
    ...
    .Notes
    Name: Get-UrlAuthorizationRules.ps1
...
ForEach ($sComputer in $global:aComputers)
{
	...
	$oCollection = Get-WmiQuery -Namespace 'root\WebAdministration' -Query 'SELECT * FROM AuthorizationRule' -Computer $sComputer
	...
	If ($oCollection -ne $null)
	{
		ForEach ($oInstance in $oCollection)
		{
			...
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Roles' -Value $([string] $oInstance.Roles)
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Users' -Value $([string] $oInstance.Users)
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Verbs' -Value $([string] $oInstance.Verbs)
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'AccessType' -Value $([Int32] $oInstance.AccessType)
			$aObjects += @($oObject)
		}
	}
	Else
...
$aObjects | Export-Csv -Path '.\Get-UrlAuthorizationRules.csv' -NoTypeInformation
Write-Host 'Done!'