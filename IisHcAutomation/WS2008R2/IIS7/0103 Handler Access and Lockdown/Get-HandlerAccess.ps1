<#
    ...
    .EXAMPLE
    .\Get-HandlerAccess.ps1 -Computers Web01;Web02;Web03
    This will gather the WMI data from Web01, Web02, and Web03 using the credentials you are current logged in with.
    .EXAMPLE
    .\Get-HandlerAccess.ps1 -Computers Web01;Web02;Web03 -User 'contoso\administrator'
    ...
    .EXAMPLE
    .\Get-HandlerAccess.ps1 -Computers Web01;Web02;Web03 -User 'contoso\administrator' -Password 'LetMeIn123'
...
{
	Write-Host "Getting HandlersSection data from $sComputer..." -NoNewline; Start-Timer
	$oCollection = Get-WmiQuery -Namespace 'root\WebAdministration' -Query 'SELECT * FROM HandlersSection' -Computer $sComputer
    Write-Host "Done! [$(Stop-Timer)]"
	
	ForEach ($oInstance in $oCollection)
	{
		$aAccessPolicy = Convert-IISAccessPolicyMaskToStringArray -Mask $oInstance.AccessPolicy
		$sAccessPolicy = [string]::Join(',',$aAccessPolicy)
		...
		Start-Timer; Write-Host "Getting $sPath file data from $sComputer..." -NoNewline
		$aFiles = Get-FilesFromAppHostPath -Computer $sComputer -AppHostPath $($oInstance.Path)
		$dDuration = Stop-Timer; Write-Host "Done! [$dDuration]"
				
		$sPhysicalFilePath = Get-PhysicalPathFromAppHost -AppHostPath $($oInstance.Path) -Computer $sComputer
		$sPhysicalFilePath = Expand-EnvironmentVariables -Computer $sComputer -String $sPhysicalFilePath
		ForEach ($oHandler in $oInstance.Handlers)
		{
			$oObject = New-Object pscustomobject

			$sRequiredAccess = Convert-IisHandlerRequiredAccessToStringArray -Mask $($oHandler.RequireAccess)				
			If ($sRequiredAccess -eq 'None')
			{
				$IsAllowed = $true
			}
			Else
			{
				$IsAllowed = $($sAccessPolicy.Contains($sRequiredAccess))
			}
			
			If ($($oHandler.Path) -ne $null)
			{
				$sExt = $($oHandler.Path).Replace('*.','')
				$aFilesWithExtension = @($aFiles | Where-Object {($_.Extension -eq $sExt)} | Select Name, Extension)
			}
			
			If ($aFilesWithExtension -eq $null)
			{
				$iCount = 'Not checked'
			}
			Else
			{
				$iCount = $aFilesWithExtension.Count
			}
			...
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Handler' -Value $($oHandler.Name)
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Ext' -Value $($oHandler.Path)
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'RequiredAccess' -Value $sRequiredAccess
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name 'Enabled' -Value $IsAllowed
			Add-Member -InputObject $oObject -MemberType NoteProperty -Name '#OfFilesWithExt' -Value $iCount
			$aObjects += @($oObject)
		}
	}
}
$aObjects | Format-Table -AutoSize
$aObjects | Export-Csv -Path '.\Get-HandlerAccess.csv' -NoTypeInformation
Write-Host 'Done!'