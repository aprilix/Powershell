<#
    .SYNOPSIS
    Gets the instances of Win32_Computer for all of the servers.
    .DESCRIPTION
    Gets the instances of Win32_ComputerSystem for all of the servers. This scripts uses the root\cimv2 WMI namespace. Requires administrator rights on the target server(s). The WMI connections are encrypted when possible.
    .EXAMPLE
    .\Get-ComputerSystem.ps1 -Computers Web01;Web02;Web03
    This will gather the WMI data from Web01, Web02, and Web03 using the credentials you are current logged in with.
    .EXAMPLE
    .\Get-ComputerSystem.ps1 -Computers Web01;Web02;Web03 -User 'contoso\administrator'
    ...
    .EXAMPLE
    .\Get-ComputerSystem.ps1 -Computers Web01;Web02;Web03 -User 'contoso\administrator' -Password 'LetMeIn123'
...
ForEach ($sComputer in $global:aComputers)
{
	...
	[string] $sWmiClass = 'Win32_ComputerSystem'
			...
			If ($oWmiInstance -ne $null)
			{
				...
				ForEach ($oWmiProperty in $($oWmiClass.Properties))
				{
					...
					switch ($sInstancePropertyType)
					{
						...
						'DateTime'
						{
							If ($($oWmiInstance.$($oWmiProperty.Name)) -ne $null)
							{
							...
							Else
							{
								Add-Member -InputObject $oNewObject1 -MemberType NoteProperty -Name $sInstancePropertyName -Value $null
							}
						}
						'Object'
						{
						...
						'SInt32' 
						{
							Add-Member -InputObject $oNewObject1 -MemberType NoteProperty -Name $sInstancePropertyName -Value $([System.Int64] $oWmiInstance.$($oWmiProperty.Name))
						}								
						'String' 
						{
...
$aWmiObjects
$aWmiObjects | Export-Csv -Path '.\Get-ComputerSystem.csv' -NoTypeInformation