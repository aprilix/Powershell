...
#>
param([string]$Computers="$env:computername",[string]$User='',[string]$Password='',[int]$CollectionInterval=60,[string]$OutputFilePath='.\EphemeralPortStats.log')
...
Function Get-EphemeralPortStats
{
	...
	{	
		...
		$aDynamicPortRangeConnections = $oActiveConnections | Where-Object -FilterScript {($_.LocalPort -gt $iDynamicStartPort) -and ($_.LocalPort -lt $iDynamicEndPort)}
		...
		ForEach ($oUniqueLocalAddress in $aUniqueLocalAddresses)
		{
			If ($($oUniqueLocalAddress.LocalAddress) -ne '0.0.0.0')
			{
				...
				$aIpEphemeralPortConnections = @($aDynamicPortRangeConnections | Where-Object -FilterScript {($_.LocalAddress -eq $sUniqueLocalAddress)} | Select LocalPort | Sort-Object | Get-Unique -AsString)
				...
				{	
					...
					Add-Member -InputObject $oStats -MemberType NoteProperty -Name '#OfEPortsInUse' -Value $([System.Int32] $aIpEphemeralPortConnections.Count)
					Add-Member -InputObject $oStats -MemberType NoteProperty -Name 'Max#OfEPorts' -Value $([System.Int32] $oDynamicPortRange.NumberOfPorts)
					...
					Add-Member -InputObject $oStats -MemberType NoteProperty -Name '%EPortUsage' -Value $iPercentage
				}
				Else
				{
					$oStats = New-Object System.Object
					...
					Add-Member -InputObject $oStats -MemberType NoteProperty -Name 'DateTime' -Value $(Get-Date)
					Add-Member -InputObject $oStats -MemberType NoteProperty -Name LocalAddress -Value $sUniqueLocalAddress
					Add-Member -InputObject $oStats -MemberType NoteProperty -Name '#OfEPortsInUse' -Value 0
					Add-Member -InputObject $oStats -MemberType NoteProperty -Name 'Max#OfEPorts' -Value $([System.Int32] $oDynamicPortRange.NumberOfPorts)
					$iPercentage = 0
					Add-Member -InputObject $oStats -MemberType NoteProperty -Name '%EPortUsage' -Value $iPercentage		
				}
				...
				{	
					Add-Member -InputObject $oStats -MemberType NoteProperty -Name '#OfTcpListeningPorts' -Value $([System.Int32] $aIpListeningPorts.Count)
				}
				Else
				{
					Add-Member -InputObject $oStats -MemberType NoteProperty -Name '#OfTcpListeningPorts' -Value 0
				}
				$aLocalAddressStats += $oStats
				
				#// Number of PIDs
				$aIpPids = $oActiveConnections | Where-Object -FilterScript {($_.LocalAddress -eq $sUniqueLocalAddress) -or ($_.LocalAddress -eq '0.0.0.0')} | Select PID | Sort-Object PID | Get-Unique -AsString
				If ($aIpPids -ne $null)
				{	
					Add-Member -InputObject $oStats -MemberType NoteProperty -Name '#OfPids' -Value $([System.Int32] $aIpPids.Count)
				}
				Else
				{
					Add-Member -InputObject $oStats -MemberType NoteProperty -Name '#OfPids' -Value 0
				}
			}
		}
	}
