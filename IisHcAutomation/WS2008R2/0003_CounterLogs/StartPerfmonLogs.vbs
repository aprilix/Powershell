'// StartPerfmonLogs.vbs
'// Purpose: This script is designed for use in Health Checks.
'//  It starts existing perfmon logs on multiple servers.
'//
'// Syntax:
...
Sub StartPerfmonLogs()
    ...
    For i = 0 to UBound(g_aComputers)
        ...
        sCommand = "logman update " & sPerfmonLogName & " -s " & sComputer & " -m start"
        WScript.echo sCommand
        Set objExecObject = objShell.Exec(sCommand)
        sStdOut = objExecObject.StdOut.ReadAll()
        WScript.Echo sStdOut

        sCommand = "logman update " & sPerfmonLogName & " -s " & sComputer & " -m stop"
        WScript.echo sCommand
        Set objExecObject = objShell.Exec(sCommand)
        sStdOut = objExecObject.StdOut.ReadAll()
        WScript.Echo sStdOut

        sCommand = "logman start " & sPerfmonLogName & " -s " & sComputer
        WScript.echo sCommand
        Set objExecObject = objShell.Exec(sCommand)
        sStdOut = objExecObject.StdOut.ReadAll()
        WScript.Echo sStdOut
        ...
        If Instr(1, sStdOut, "command completed successfully", 1) > 0 Then
            ...
            newClassNode.SetAttribute "Name", "StartPerfmonLogs.vbs"
...
Sub ProcessArguments()
    sSyntax = "" &_
    "CScript StartPerfmonLogs.vbs <computer[;computer]> <PerfmonLogNamePrefix>" & vbNewLine &_
