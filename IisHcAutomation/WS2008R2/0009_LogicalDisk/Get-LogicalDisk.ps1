<#
    ...
    .EXAMPLE
    .\Get-LogicalDisk.ps1 -Computers Web01;Web02;Web03
    This will gather the WMI data from Web01, Web02, and Web03 using the credentials you are current logged in with.
    .EXAMPLE
    .\Get-LogicalDisk.ps1 -Computers Web01;Web02;Web03 -User 'contoso\administrator'
    ...
    .EXAMPLE
    .\Get-LogicalDisk.ps1 -Computers Web01;Web02;Web03 -User 'contoso\administrator' -Password 'LetMeIn123'
...
{		
	...
	[string] $sWmiClass = 'Win32_LogicalDisk'
	...
	If ($oWmiCollectionOfInstances -ne $null)
	{
		ForEach ($oWmiInstance in $oWmiCollectionOfInstances)
		{
			If ($oWmiInstance -ne $null)
			{
				...
				#// PROPERTIES
				If ($(Test-Property -InputObject $oWmiClass -Name 'Properties') -eq $true)
				{
				...
				Else
				{
					ForEach ($oWmiProperty in $(Get-Member -InputObject $oWmiInstance -MemberType *Property))
					{
						If ($($oWmiProperty.Name.IndexOf('__')) -ne 0)
						{
							Add-Member -InputObject $oNewObject1 -MemberType NoteProperty -Name $oWmiProperty.Name -Value $($oWmiInstance.$($oWmiProperty.Name))
						}
					}					
				}
...
$aWmiObjects | Export-Csv -Path '.\Get-LogicalDisk.csv'